...
D-HASH 알고리즘의 성능 임계치를 검증하기 위해 부하 테스트를 수행하던 중, Redis 서버의 자원은 널널함에도 불구하고 클라이언트의 **처리량(Throughput)**이 특정 구간에서 정체되는 것을 확인했다.

이 현상은 서버 성능이 부족해서가 아니라 부하를 생성하는 벤치마크 툴 자체가 느려서 발생하는 **클라이언트 사이드 병목(Client-side Bottleneck)** 때문에 발생한다.
...
나는 이 병목을 해소하기 위해 Python의 `concurrent.futures.ThreadPoolExecutor`를 도입하여 아키텍처를 **비동기 병렬 처리** 방식으로 수정했다. Python의 GIL(Global Interpreter Lock)은 CPU 연산 병렬화를 제한하지만, Socket 통신과 같은 Blocking I/O 대기 중에는 GIL을 해제한다. 따라서 노드 통신과 같은 I/O Bound 작업에서는 멀티 스레딩을 통해 완벽한 병렬 부하 주입이 가능하다고 판단했다.
...
## 18만 OPS 달성 및 테스트 환경의 한계

테스트 환경 전환 후 벤치마크를 수행한 결과, 기존에서는 나오지 못했던 **$160,000 \sim 180,000$ OPS** 수준의 처리량이 나왔다. 이는 클라이언트 내부의 대기 시간(Queueing Delay)을 제거하여 순수한 서버의 능력을 이끌어낸 성과다.

이 방식은 현재 Python 생태계 내에서는 최적의 모델이지만 **스레드 컨텍스트 스위칭 오버헤드**라는 문제가 있다. 현재 수준의 수십만 단위 트래픽 시뮬레이션에서는 동작하지만, 만약 수백만 OPS 이상의 트래픽 환경에서의 테스트가 요구된다면 `memtier_benchmark`와 같은 C/C++ 기반 전용 벤치마크 도구로 인프라를 이전해야한다.